...
export const sendEmail = async ({ to, subject, html, attachments }: EmailOptions) => {
    try {
        const info = await transporter.sendMail({
            from: `"DigitBill" <${process.env.SMTP_USER}>`,
            to,
            subject,
            html,
            attachments
        ...
        throw new Error('No se pudo enviar el correo electrónico. Verifica la configuración SMTP.');
    }
};

export const sendInvoiceEmail = async (clientEmail: string, invoice: any, company: any) => {
    const formattedTotal = new Intl.NumberFormat('es-DO', { style: 'currency', currency: 'DOP' }).format(invoice.total);
    ...
    <body>
        <div className="container">
            ...
            <div className="invoice-details">
                ...
                <p>Te enviamos la factura correspondiente a tu reciente transacción con <span className="highlight">${company.name}</span>.</p>
                
                <table style="width: 100%; border-collapse: collapse; margin-top: 20px;">
                    <tr>
                        <td style="padding: 8px 0; color: #666;">NCF:</td>
                        <td style="padding: 8px 0; text-align: right; font-weight: bold;">${invoice.e_ncf || 'B00000000'}</td>
                    </tr>
                    <tr>
                        <td style="padding: 8px 0; color: #666;">Número:</td>
                        <td style="padding: 8px 0; text-align: right;">${invoice.sequential_number.toString().padStart(6, '0')}</td>
                    </tr>
                    <tr>
            ...
            <div className="footer">
                <p>Enviado automáticamente por DigitBill por encargo de ${company.name}.</p>
    ...
    return sendEmail({
        to: clientEmail,
        subject: `Factura ${invoice.e_ncf || '#' + invoice.sequential_number} - ${company.name}`,
        html
    });
};
