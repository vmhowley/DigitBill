...
router.put("/company", async (req, res) => {
  try {
    ...
    if (electronic_invoicing !== undefined) {
      ...
      const isEnabling =
        electronic_invoicing === true || electronic_invoicing === "true";

      if (
        isEnabling &&
        !limits.features.includes("Electronic Invoicing") &&
        !limits.features.includes("All")
      ) {
        return res.status(403).json({
          error: `Tu plan ${req.plan?.toUpperCase()} no incluye Facturación Electrónica. Actualiza al plan Pyme.`,
        });
      }

      await configService.updateCompanyConfig(
        req.tenantId!,
        "electronic_invoicing",
        electronic_invoicing.toString()
      );
    }
...
router.post("/users/invite", async (req, res) => {
  try {
    ...
    const existingUser = await query(
      "SELECT id FROM users WHERE email = $1 OR supabase_uid = $2",
      [email, supabaseUid]
    );
    ...
    if (existingUser.rows.length > 0) {
      return res
        .status(400)
        .json({ error: "User already exists in this system" });
    }
...
router.post(
  ...
  async (req: any, res) => {
    try {
      ...
      console.log(
        `Certificate uploaded for tenant ${req.tenantId}: ${filePath}`
      );
      ...
      res
        .status(500)
        .json({ error: "Error al subir el certificado: " + err.message });
    }
  }
);
