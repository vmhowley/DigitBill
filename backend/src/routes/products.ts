...
router.get("/", async (req, res) => {
  try {
    const result = await query(
      "SELECT * FROM products WHERE tenant_id = $1 ORDER BY description ASC",
      [req.tenantId]
    );
...
router.post("/", requireRole(["admin"]), async (req, res) => {
  try {
    ...
    // Initialize with 0, then add movement if needed
    const initialStock = productType === "service" ? 0 : stock_quantity || 0;
    ...
    if (initialStock > 0) {
      try {
        await inventoryService.recordMovement(req.tenantId!, {
        ...
        newProduct.stock_quantity = initialStock;
      } catch (moveErr) {
        console.error("Error creating initial stock movement:", moveErr);
        // Non-fatal, return product but log error
      }
    }
    ...
    res
      .status(500)
      .json({ error: "Error creating product", details: (err as any).message });
  }
});
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    const invoices = await query(
      "SELECT id FROM invoice_items WHERE product_id = $1 AND tenant_id = $2 LIMIT 1",
      [id, req.tenantId]
    );
    if (invoices.rows.length > 0) {
      return res
        .status(400)
        .json({ error: "Cannot delete product used in existing invoices" });
    }
